describe('Sign In and Check Elements', () => {
  ...
  it('should sign in with email and password, then check page elements', () => {
    ...
    cy.get(".sc-glhZnD > :nth-child(1)").contains("Home").should("be.visible");
    cy.get(".sc-glhZnD > :nth-child(2)")
      .contains("Drinks")
      .should("be.visible");
    ...
    cy.get(".sc-kgEbbF").should("have.attr", "src", "moon_logo.png"); // Перевірити, що змінився логотип на сонце

    // Switch to Dark theme
    cy.get(".sc-bBQvrP").click();
    //cy.wait(2000); // Pause for 2 seconds
    cy.get(".sc-bBQvrP").should("have.text", "Dark Mode");
